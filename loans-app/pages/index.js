...
export default function Home({ loanData, currentUser, nonce }) {
  ...
  const [totalInvestmentAvailable, setTotalInvestmentAvailable] = useState(
    currentUser.TOTAL_AVAILABLE
  );
  const investInLoan = (loan, user, amount) => {
    ...
    try {
      ...
      const newAmountAvailable =
        BigInt(pendingNewLoan.available.replace(",", "")) - amt;
      const newTotalInvestmentAvailable =
        BigInt(totalInvestmentAvailable) - amt;
      ...
      pendingNewLoan.amount = (
        BigInt(pendingNewLoan.amount.replace(",", "")) + amt
      ).toString();
  ...
  return (
    <div className={""}>
      ...
      <div className="bg-gray-200 h-screen">
        ...
        <main className={""}>
          <h1 className="text-3xl font-bold w-4/6 mx-auto mb-10">
            Current Loans
          </h1>
          <CurrentLoanList
        ...
        <footer className={""}>written by abdoul sy - version {VERSION}</footer>
      </div>
    </div>
  );
}
